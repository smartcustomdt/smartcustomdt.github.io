...
gulp.task('vendor:temp', () => {
  ...
  return gulp.src(modules, { cwd: 'node_modules', base: './node_modules' }).pipe(gulp.dest('temp'));
});
  ...
  Object.keys(vendors).forEach(vendor => {
    ...
    if (typeof src === 'object') {
      if (isIterableArray(src)) {
        src.forEach(file => {
          addToPromises(`./node_modules/${vendor}/${file}`, `${baseDir}/vendors/${dest}`);
        });
      } else {
        ...
        destDirectories.forEach(dir => {
          if (isIterableArray(vendors[vendor].src[dir])) {
            vendors[vendor].src[dir].forEach(file => {
              addToPromises(`./node_modules/${vendor}/${file}`, `${baseDir}/vendors/${dest}/${dir}`);
            });
          } else {
            addToPromises(`./node_modules/${vendor}/${vendors[vendor].src[dir]}`, `${baseDir}/vendors/${dest}/${dir}`);
          }
        });
      }
    } else {
      addToPromises(`./node_modules/${vendor}/${src}`, `${baseDir}/vendors/${dest}`);
    }
  });
...
gulp.task('vendor:clean', () => {
  const directories = Object.keys(vendors).map(vendor => `${baseDir}/vendors/${vendors[vendor].dest}`);
